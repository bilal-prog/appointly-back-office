...
export function CustomersClient() {
  ...
  const query = useQuery({
    queryKey: ["customers", offset, PAGE_SIZE],
    queryFn: async () =>
      (
        await clientApi.get<CustomersResponse>("/customers", {
  ...
  return (
    <div className="space-y-4">
      ...
      {customers.length ? (
        <DataTable
          headers={["Name", "Email", "Appointments", "Last appointment", "Status"]}
          ...
          {customers.map((customer) => (
            ...
            >
              ...
              <td className="px-4 py-3">
                {format(new Date(customer.lastAppointmentDate), "PP")}
              </td>
              <td className="px-4 py-3">
      ...
      <Dialog open={Boolean(selected)} onOpenChange={() => setSelected(null)}>
        <DialogContent>
          ...
          <div className="rounded-md border bg-muted/40 p-4 text-sm">
            Appointment history will appear here when the endpoint is available.
          </div>
        </DialogContent>
      </Dialog>
    </div>
